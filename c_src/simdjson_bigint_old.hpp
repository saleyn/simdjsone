...
// Original BigInt implementation (for comparison)
struct BigIntOld {
  // Original decode implementation with 255 byte limitation
  static ERL_NIF_TERM
  ...
  // Simple encode that just fails for non-small integers (original behavior)
  static std::string
  encode(ErlNifEnv* env, ERL_NIF_TERM term)
  {
    // Original implementation didn't have encoding - would just fail
    return {};
  }
